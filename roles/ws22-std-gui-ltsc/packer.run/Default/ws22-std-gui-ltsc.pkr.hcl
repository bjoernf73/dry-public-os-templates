...
source "vsphere-iso" "ws22-std-gui-ltsc" {
  CPUs                 = 4
  NestedHV             = true
  RAM                  = 4096
...
build {
  sources = ["source.vsphere-iso.ws22-std-gui-ltsc"]
  ...
  provisioner "powershell" {
    elevated_password = "${var.winrm_password}"
    elevated_user     = "${var.winrm_username}"
    inline            = ["f:\\win-scripts\\ConfigPreReboot.ps1 -Configfile f:\\ws22-std-gui-ltsc-OSConfig.json"]
    max_retries       = "3"
    pause_before      = "2s"
  }
  ...
  provisioner "powershell" {
    elevated_password = "${var.winrm_password}"
    elevated_user     = "${var.winrm_username}"
    inline            = ["f:\\win-scripts\\ConfigPostReboot.ps1 -Configfile f:\\ws22-std-gui-ltsc-OSConfig.json"]
    max_retries       = "3"
  }
  ...
  provisioner "powershell" {
    elevated_password = "${var.winrm_password}"
    elevated_user     = "${var.winrm_username}"
    inline            = ["f:\\win-scripts\\ConfigUpdates.ps1 -Configfile f:\\ws22-std-gui-ltsc-OSConfig.json"]
    max_retries       = "2"
    timeout           = "2h0m0s"
  }
  ...
  provisioner "powershell" {
    elevated_password = "${var.winrm_password}"
    elevated_user     = "${var.winrm_username}"
    inline            = ["f:\\win-scripts\\ConfigLast.ps1 -Configfile f:\\ws22-std-gui-ltsc-OSConfig.json"]
    max_retries       = "2"
  }
}